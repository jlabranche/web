...
$.fn.board_task = function(task)
{
    return this.each(function()
    {
        ...
        $task.on( 'keyup', '.editable-input',
            function(e) {
                ...
                if(e.keyCode === 27) {
                    ...
                    // restore prev value
                    $input.val(orig).prop('readonly', true);
                }
            }
        ); // keyup
        ...
        function save_description ($div) {
            // prevent saving twice, if they blur instead of timeout
            clearTimeout($div.data('save_timer'));
            encode_urls_emails ($div);

            // store prev 
            var prev_desc = task.desc + '';
            var new_desc = $div.html();
            var comment = board.text()['task_description_updated'].sprintf(
                board.current_user().short_name,
                new_desc
            );

            if ( prev_desc !== '' ) {
                comment += board.text()['task_description_updated_previous'].sprintf(
                    prev_desc
                );
            }

            if ( new_desc === '' || new_desc  === prev_desc ) {
                comment = null;
            }
        ...
        $('.task-title', '.task-description', $task).on( 'keydown',
            function(e) {
                if ( !current_user_has_cap('write') ) {
                    return false;
                }
        ...
        $('.task-description', $task).on( 'keyup',
            function() {
                if ( !current_user_has_cap('write') ) {
                    return false;
                }
        ...
        // assign task to user
        $task.on( 'click', '.dropdown-menu-allowed-users .btn',
            function() {
                var $btn = $(this);
                var user_id = $btn.val();
